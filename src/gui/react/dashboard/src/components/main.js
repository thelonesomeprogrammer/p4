	...
	useEffect(() => {
		const interval = setInterval(async () => {
			try {
				...
				const x = textdata[0].split("x:")[1].split(";")[0];
				const y = textdata[0].split("y:")[1].split(";")[0];
				const z = textdata[0].split("z:")[1].split(";")[0];

				setData((prevData) => ({
					x: [...prevData.x.slice(-50), Number.parseFloat(x)],
					y: [...prevData.y.slice(-50), Number.parseFloat(y)],
					z: [...prevData.z.slice(-50), Number.parseFloat(z)],
				}));
