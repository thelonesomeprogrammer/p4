...
// stabilizerStep_t represents the number of times the stabilizer loop has run
// (at 1000 Hz)
...
typedef struct state_s {
  attitude_t attitude; // deg (legacy CF2 body coordinate system, where pitch is
                       // inverted)
...
typedef enum control_mode_e {
  controlModeLegacy =
      0, // legacy mode with int16_t roll, pitch, yaw and float thrust
...
typedef struct control_s {
  union {
    ...
    // different platforms. The normalized force API is probably a better
    // option.
    struct {
      float thrustSi; // N
      union {         // Nm
